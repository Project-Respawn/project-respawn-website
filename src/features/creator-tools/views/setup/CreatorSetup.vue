...
export default {
  ...
  methods: {
    ...
    async refreshSetupState() { this.loading = true; this.error = ''; try { this.applyAccess(await refreshAccessContext({ force: true })); await this.refreshTwitchStatus(); } catch (error) { this.error = error?.message || 'Could not load creator setup.'; } finally { this.loading = false; } },
    ...
    async refreshTwitchStatus() { this.twitchConnected = false; this.twitchAccountName = ''; if (!this.selectedBrandId) return; try { const status = await getTwitchConnectionStatus(client, this.selectedBrandId); this.twitchConnected = status.connected; this.twitchAccountName = status.accountName; } catch (error) { this.error = error?.message || 'Could not load Twitch status.'; } },
