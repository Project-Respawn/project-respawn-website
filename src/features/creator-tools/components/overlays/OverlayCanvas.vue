  ...
  <div ref="viewport" class="canvas-viewport" :class="{preview:previewMode}" @pointerdown.self="$emit('select','')" @dragstart.prevent>
    <div class="canvas-scaler" :style="{width:`${overlay.resolution.width*scale}px`,height:`${overlay.resolution.height*scale}px`}">
      <div ref="canvas" class="overlay-canvas" :style="canvasStyle">
        ...
        <div v-for="widget in ordered" v-show="widget.enabled" :key="widget.id" class="widget-frame" :class="{selected:widget.id===selectedId,locked:widget.locked,dragging:widget.id===draggingId,replaying:widget.id===activeWidgetId&&!animationsPaused}" :data-instance-id="widget.id" :data-locked="widget.locked" :style="frameStyle(widget)" :tabindex="previewMode?-1:0" :aria-label="`${widget.name}${widget.locked?', locked':''}`" @pointerdown.stop.prevent="startInteraction($event,widget)" @pointermove="updateHover($event,widget)" @pointerleave="clearHover(widget.id)" @keydown="nudge($event,widget)">
...
import{canvasScalesFromBounds,clientDeltaToCanvas,DEFAULT_GRID_SIZE,detectResizeDirection,editorScale,moveFrame,resizeFrameWithSnapping}from'../../overlays/overlayGeometry.js'
...
const emit=defineEmits(['select','change']),viewport=ref(null),canvas=ref(null),scale=ref(.45),snapGuides=ref([]),draggingId=ref(''),hovered=ref({id:'',direction:null});let observer,interaction=null
...
function resizeCursor(direction){return direction?({n:'ns-resize',s:'ns-resize',e:'ew-resize',w:'ew-resize',nw:'nwse-resize',se:'nwse-resize',ne:'nesw-resize',sw:'nesw-resize'}[direction]):null}
function frameCursor(w){if(props.previewMode)return'default';if(w.locked)return'not-allowed';if(draggingId.value===w.id)return resizeCursor(interaction?.handle)||'grabbing';return resizeCursor(hovered.value.id===w.id?hovered.value.direction:null)||(definition(w).capabilities.draggable===false?'default':'grab')}
function frameStyle(w){return{left:`${w.frame.x}px`,top:`${w.frame.y}px`,width:`${w.frame.width}px`,height:`${w.frame.height}px`,zIndex:w.zIndex,cursor:frameCursor(w),...themeVariables(w.themeId||props.overlay.themeId)}}
function definition(w){return registry[w.type]||{capabilities:{draggable:true,resizable:true},minimumSize:{width:80,height:50}}}
function begin(e,data){if(e.isPrimary===false||e.button!==0||interaction)return;const captureTarget=e.currentTarget,canvasBounds=canvas.value?.getBoundingClientRect();if(!captureTarget||!canvasBounds)return;const scales=canvasScalesFromBounds(canvasBounds,props.overlay.resolution);if(!scales)return;e.preventDefault();captureTarget.setPointerCapture?.(e.pointerId);interaction={...data,pointerId:e.pointerId,captureTarget,scaleX:scales.x,scaleY:scales.y,startX:e.clientX,startY:e.clientY,changed:false};draggingId.value=data.widget.id;add()}
function pointerDirection(e,w){if(props.previewMode||w.locked||definition(w).capabilities.resizable===false)return null;return detectResizeDirection(e.clientX,e.clientY,e.currentTarget.getBoundingClientRect(),9)}
function startInteraction(e,w){emit('select',w.id);if(props.previewMode||w.locked)return;const handle=pointerDirection(e,w);if(handle)begin(e,{kind:'resize',handle,widget:createWidgetSnapshot(w)});else if(definition(w).capabilities.draggable!==false)begin(e,{kind:'move',widget:createWidgetSnapshot(w)})}
function updateHover(e,w){if(interaction)return;hovered.value={id:w.id,direction:pointerDirection(e,w)}}
function clearHover(id){if(!interaction&&hovered.value.id===id)hovered.value={id:'',direction:null}}
...
function finishInteraction(commit){if(!interaction)return;const active=interaction,w=props.overlay.widgets.find(x=>x.id===active.widget.id);if(commit&&w&&active.changed)emit('change',createWidgetSnapshot(w),true);if(active.captureTarget?.hasPointerCapture?.(active.pointerId))active.captureTarget.releasePointerCapture(active.pointerId);interaction=null;draggingId.value='';hovered.value={id:'',direction:null};snapGuides.value=[];remove()}
