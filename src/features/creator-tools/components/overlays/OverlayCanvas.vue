    ...
    <div class="canvas-scaler" :style="{width:`${overlay.resolution.width*scale}px`,height:`${overlay.resolution.height*scale}px`}">
      <div ref="canvas" class="overlay-canvas" :style="canvasStyle">
        ...
        <div v-for="widget in ordered" v-show="widget.enabled" :key="widget.id" class="widget-frame" :class="{selected:widget.id===selectedId,locked:widget.locked,dragging:widget.id===draggingId,replaying:widget.id===activeWidgetId&&!animationsPaused}" :data-instance-id="widget.id" :data-locked="widget.locked" :style="frameStyle(widget)" :tabindex="previewMode?-1:0" :aria-label="`${widget.name}${widget.locked?', locked':''}`" @pointerdown.stop.prevent="startMove($event,widget)" @click.stop="$emit('select',widget.id)" @keydown="nudge($event,widget)">
          ...
          <button v-if="!previewMode&&widget.id===selectedId&&widget.type==='twitch-chat'" class="chat-drag-handle" type="button" :disabled="widget.locked" aria-label="Drag Chat" :title="widget.locked?'Unlock Unified Chat to reposition it':'Drag to reposition'" @pointerdown.stop.prevent="startMove($event,widget)"><span aria-hidden="true">⠿</span><b>Drag Chat</b></button>
          <template v-if="!previewMode&&widget.id===selectedId&&!widget.locked"><button v-for="handle in handles" :key="handle" class="resize-handle" :class="handle" :aria-label="`Resize ${widget.name} from ${handle}`" @pointerdown.stop.prevent="startResize($event,widget,handle)"></button></template>
...
function startMove(e,w){emit('select',w.id);if(props.previewMode||w.locked)return;if(w.type==='twitch-chat'&&!e.currentTarget?.classList?.contains('chat-drag-handle'))return;begin(e,{kind:'move',widget:createWidgetSnapshot(w)})}
function startResize(e,w,handle){begin(e,{kind:'resize',handle,widget:createWidgetSnapshot(w)})}
...
function onMove(e){if(!interaction||e.pointerId!==interaction.pointerId)return;e.preventDefault();const dx=e.clientX-interaction.startX,dy=e.clientY-interaction.startY;if(!interaction.changed&&Math.hypot(dx,dy)<2)return;interaction.changed=true;const d=clientDeltaToCanvas(dx,dy,interaction.scaleX,interaction.scaleY);let frame;if(interaction.kind==='move'){const moved=moveFrame(interaction.widget.frame,d,props.overlay.resolution,props.overlay.widgets.filter(w=>w.id!==interaction.widget.id).map(w=>w.frame),props.snapping);frame=moved.frame;snapGuides.value=moved.guides}else frame=resizeFrame(interaction.widget.frame,interaction.handle,d,props.overlay.resolution);emit('change',{...interaction.widget,frame},false)}
