...
test('canvas exposes universal dragging, eight resize handles, and locked fallbacks', () => {
  ...
  assert.match(canvas, /@pointerdown\.stop\.prevent="startResize\(\$event,widget,handle\)"/)
  assert.match(canvas, /type="button" tabindex="-1" class="resize-handle"/)
...
test('canvas interaction CSS uses invisible resize zones and a frame-filling renderer', () => {
  const root = fileURLToPath(new URL('../../', import.meta.url))
  ...
  assert.match(css, /\.resize-handle \{[\s\S]*?background:\s*transparent;[\s\S]*?opacity:\s*0;/)
  assert.match(css, /\.resize-handle\.n,[\s\S]*?cursor: ns-resize;/)
  assert.match(css, /\.resize-handle\.e,[\s\S]*?cursor: ew-resize;/)
  assert.match(css, /\.resize-handle\.ne[\s\S]*?cursor:\s*nesw-resize;/)
})
