...
export function routeDemoEvent(scene,event){const widget=scene?.widgets.find(w=>w.enabled&&(ALERT_WIDGETS[w.type] ? ALERT_WIDGETS[w.type].topic===event.type : w.type===event.targetWidgetType || w.type==='alerts'&&Object.values(ALERT_WIDGETS).some(alert=>alert.topic===event.type)));return widget?{ok:true,widgetId:widget.id,message:`Replaying ${event.summary}` }:{ok:false,widgetId:'',message:`This scene does not currently contain a ${event.targetWidgetType.replaceAll('-',' ')} widget.`}}
