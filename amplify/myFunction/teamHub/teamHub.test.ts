...
test('dual SuperAdmin Manager retains platform and team capabilities while plain Admin lacks manager capabilities', async () => {
  ...
  memberships.push({ id: `team-membership:${team.id}:${IDS.admin}`, teamId: team.id, userId: IDS.admin, displayName: 'Dual admin', role: 'MANAGER', status: 'ACTIVE' })
  const dual = await handleGetTeamHub(event(IDS.admin, ['SuperAdmin'], { teamId: team.id }), client)
  assert.equal(dual.isPlatformAdmin, true); assert.equal(dual.teamRole, 'MANAGER')
  ...
  memberships.pop()
  const plain = await handleGetTeamHub(event(IDS.admin, ['SuperAdmin'], { teamId: team.id }), client)
  assert.equal(plain.isPlatformAdmin, true); assert.equal(plain.teamRole, null); assert.equal(plain.capabilities.canManageMembers, false)
  await assert.rejects(() => handleManageTeamMember(event(IDS.admin, [], { teamId: team.id, targetEmail: 'player@example.com', role: 'PLAYER', action: 'ASSIGN', expectedRevision: 4 }), client, { transact: async () => undefined }, names), /denied/)
})
