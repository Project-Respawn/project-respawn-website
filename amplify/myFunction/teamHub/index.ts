...
export async function handleSearchTeamAssignableUsers(event: any, injected?: any) {
  ...
  if (!current.isAdmin) {
    const data = await client(injected), teamId = teamIdValue(event.arguments?.teamId)
    await teamFor(data, teamId)
    const memberships = await membershipsFor(data, teamId)
    requireManager(memberships, current.userId)
  }
...
export async function handleGetTeamHub(event: any, injected?: any) {
  ...
  const members = await membershipsFor(data, team.id); requireTeamAccess(members, current.userId, current.isAdmin)
  if (!current.isAdmin && team.status !== ACTIVE) fail(TEAM_HUB_DENIED)
  const mine = members.find((row: any) => row.userId === current.userId && row.status === ACTIVE && row.teamId === team.id)
  const teamRole = mine?.role || null
  const canManage = current.isAdmin || teamRole === 'MANAGER'
